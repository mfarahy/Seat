    ...
    $.widget("ui.dialogExtend", {
        ...
        _saveSnapshot: function () {
            if (this._state === "normal") {
                ...
                // edited by mfarahy bug fixed for fixed dialogs
                var position = $(this.element[0]).dialog("widget").css('position');
                if (position == 'fixed') {
                    var pos = $(this.element[0]).dialog("widget").position();
                    this.original_position_left = pos.left;
                    this.original_position_top = pos.top;
                } else {
